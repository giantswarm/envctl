...
`~/.config/envctl/capability/definitions/`
...
The system ships with example capability definitions in `.envctl/`:
...
cp .envctl/capability-example-teleport-auth.yaml ~/.config/envctl/capability/definitions/my-auth.yaml
```
...
in envctl, allowing users to create domain-specific abstractions over MCP server tools.
*/
package capability
