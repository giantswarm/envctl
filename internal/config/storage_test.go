...
func TestStorage_Save(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedDir := filepath.Join(projectDir, ".envctl", tt.entityType)
...
func TestStorage_Load(t *testing.T) {
	...
	projectWorkflowDir := filepath.Join(projectDir, ".envctl", "workflows")
...
func TestStorage_resolveEntityDir(t *testing.T) {
	...
	tests := []struct {
		name         string
		entityType   string
		setupProject bool   // Whether to create .envctl directory
		wantContains string // What the resolved path should contain
	}{
		{
			name:         "prefer project when .envctl exists",
			entityType:   "workflows",
			setupProject: true,
			wantContains: ".envctl/workflows",
		},
		{
			name:         "fallback to user when no .envctl",
			entityType:   "workflows",
			setupProject: false,
			wantContains: ".config/envctl/workflows",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.setupProject {
				...
				envctlDir := filepath.Join(projectDir, ".envctl")
				if err := os.MkdirAll(envctlDir, 0755); err != nil {
					t.Fatalf("Failed to create .envctl directory: %v", err)
				}
			} else {
				// Remove .envctl directory if it exists
				envctlDir := filepath.Join(projectDir, ".envctl")
				os.RemoveAll(envctlDir)
			}
