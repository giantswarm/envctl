...
// NewAPIToolCaller creates a new API-based tool caller
func NewAPIToolCaller() *APIToolCaller {
	return &APIToolCaller{}
}

// CallTool implements the capability ToolCaller interface
func (atc *APIToolCaller) CallTool(ctx context.Context, toolName string, arguments map[string]interface{}) (map[string]interface{}, error) {
...
func (atc *APIToolCaller) CallToolInternal(ctx context.Context, toolName string, args map[string]interface{}) (*mcp.CallToolResult, error) {
	aggregatorHandler := GetAggregator()
	if aggregatorHandler == nil {
		return nil, fmt.Errorf("aggregator handler not available")
	}
...
func (atc *APIToolCaller) IsToolAvailable(toolName string) bool {
	aggregatorHandler := GetAggregator()
	if aggregatorHandler == nil {
		return false
	}
...
func (atc *APIToolCaller) GetAvailableTools() []string {
	aggregatorHandler := GetAggregator()
	if aggregatorHandler == nil {
		return []string{}
	}
...
type APIToolChecker struct{}

// NewAPIToolChecker creates a new API-based tool checker
func NewAPIToolChecker() *APIToolChecker {
	return &APIToolChecker{}
}

// IsToolAvailable checks if a tool is available using the aggregator API handler
func (atc *APIToolChecker) IsToolAvailable(toolName string) bool {
	aggregatorHandler := GetAggregator()
	if aggregatorHandler == nil {
		return false
	}
...
func (atc *APIToolChecker) GetAvailableTools() []string {
	aggregatorHandler := GetAggregator()
	if aggregatorHandler == nil {
		return []string{}
	}
