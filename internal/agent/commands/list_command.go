...
// listCoreTools lists envctl core tools by filtering tools that start with "core_"
func (l *ListCommand) listCoreTools(ctx context.Context) error {
	...
	l.output.OutputLine("Core envctl tools (%d found out of %d total):", len(coreTools), len(tools))
	l.output.OutputLine("")
