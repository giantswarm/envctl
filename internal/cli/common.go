...
func CheckServerRunning() error {
	...
	if err != nil {
		return fmt.Errorf("envctl server is not running. Start it with: envctl serve")
	}
	defer resp.Body.Close()

	// For streamable-http MCP, a GET request should return 202 Accepted or similar
	if resp.StatusCode != http.StatusAccepted && resp.StatusCode != http.StatusOK {
		return fmt.Errorf("envctl server is not responding correctly (status: %d). Try restarting with: envctl serve", resp.StatusCode)
	}
